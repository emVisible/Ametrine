...
from .service import DocumentService, get_document

route_document = APIRouter(prefix="/document", tags=[Tags.relation_db])
...
async def create(
    ...
    service: DocumentService = Depends(get_document),
):
...
async def all_documents(service: DocumentService = Depends(get_document)):
